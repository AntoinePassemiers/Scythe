...
cdef void enqueue(Queue* queue, void* data):
    cdef QueueNode* temp = <QueueNode*>malloc(sizeof(QueueNode))
    temp.data = data
    temp.next = NULL
    if queue.front == NULL or queue.rear == NULL:
        queue.front = temp
        queue.rear = temp
    else:
...
cdef void* dequeue(Queue* queue):
    cdef QueueNode* temp = queue.front
...
cdef bint is_queue_empty(Queue* queue):
...
cdef class ConvForest:
    ...
    cdef Py_ssize_t max_n_nodes

    cdef Node* root

    def __cinit__(self, kh, kw, nc, n_classes, max_n_nodes=10):
        self.kh = kh
        self.kw = kw
        self.nc = nc
        self.n_classes = n_classes
    ...
    def fit(self, X, Y):
        ...
        H = X.shape[1]
        W = X.shape[2]
        sample_mask = np.full((N, H-self.kh+1, W-self.kw+1), 0)
        cdef Py_ssize_t n_nodes = 0
        cdef Node* current_node = <Node*>malloc(sizeof(Node))
        cdef Queue* queue = new_queue()
        enqueue(queue, <void*>current_node)
        while (not is_queue_empty(queue)) and (n_nodes < self.max_n_nodes):
            current_node = <Node*>dequeue(queue)
            f = np.random.randint(0, 255, size=1)[0] # TODO
            n_nodes += 1

    def scan(self, data_t[:, :, :] x, label_t[:, :, :] y, mask_t[:, :] sample_mask, Py_ssize_t f, data_t threshold):
        assert(x.shape[0] == y.shape[0] and x.shape[1] == y.shape[1])
        cdef cnp.int_t[:] counters = np.zeros(self.n_classes, dtype=np.int)
